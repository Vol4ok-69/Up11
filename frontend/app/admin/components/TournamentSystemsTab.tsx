...
} from "@/lib/services/tournament-systems.service"

export default function TournamentSystemsTab() {
    const [systems, setSystems] = useState<TournamentSystemReadDto[]>([])
    ...
    async function load() {
        try {
            setError(null)
            const data = await TournamentSystemsService.getAll()
            setSystems(data)
        } catch (e: any) {
            setError(e.message)
        } finally {
    ...
    async function handleCreate() {
        ...
        try {
            await TournamentSystemsService.create({ title: newTitle })
            setNewTitle("")
            await load()
        } catch (e: any) {
            alert(e.message)
    ...
    async function handleUpdate(id: number, title: string) {
        try {
            await TournamentSystemsService.update(id, { title })
            await load()
        } catch (e: any) {
            alert(e.message)
        }
    ...
    async function handleDelete(id: number) {
        try {
            await TournamentSystemsService.delete(id)
            await load()
        } catch (e: any) {
            alert(e.message)
        }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="p-4 rounded-xl bg-slate-100 dark:bg-slate-800 space-y-3">
                <h3 className="font-semibold">Создать систему турнира</h3>
            ...
            {systems.map((system) => (
                ...
                >
                    <input
                        ...
                        onBlur={(e) =>
                            handleUpdate(system.id, e.target.value)
                        }
                    />
