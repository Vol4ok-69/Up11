...
export default function TournamentStatusesTab() {
    ...
    async function load() {
        try {
            setError(null)
            const data = await TournamentStatusesService.getAll()
            setStatuses(data)
        } catch (e: any) {
            setError(e.message)
        } finally {
    ...
    async function handleCreate() {
        ...
        try {
            await TournamentStatusesService.create({ title: newTitle })
            setNewTitle("")
            await load()
        } catch (e: any) {
            alert(e.message)
    ...
    async function handleUpdate(id: number, title: string) {
        try {
            await TournamentStatusesService.update(id, { title })
            await load()
        } catch (e: any) {
            alert(e.message)
        }
    ...
    async function handleDelete(id: number) {
        try {
            await TournamentStatusesService.delete(id)
            await load()
        } catch (e: any) {
            alert(e.message)
        }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="p-4 rounded-xl bg-slate-100 dark:bg-slate-800 space-y-3">
                <h3 className="font-semibold">Создать статус турнира</h3>
