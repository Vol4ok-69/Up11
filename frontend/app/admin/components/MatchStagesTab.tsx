...
export default function MatchStagesTab() {
    ...
    async function load() {
        try {
            setError(null)
            const data = await MatchStagesService.getAll()
            setStages(data)
        } catch (e: any) {
            setError(e.message)
        } finally {
    ...
    async function handleCreate() {
        ...
        try {
            await MatchStagesService.create({ title: newTitle })
            setNewTitle("")
            await load()
        } catch (e: any) {
            alert(e.message)
    ...
    async function handleUpdate(id: number, title: string) {
        try {
            await MatchStagesService.update(id, { title })
            await load()
        } catch (e: any) {
            alert(e.message)
        }
    ...
    async function handleDelete(id: number) {
        try {
            await MatchStagesService.delete(id)
            await load()
        } catch (e: any) {
            alert(e.message)
        }
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="p-4 rounded-xl bg-slate-100 dark:bg-slate-800 space-y-3">
                <h3 className="font-semibold">Создать этап матча</h3>
            ...
            {stages.map((stage) => (
                ...
                >
                    <input
                        ...
                        onBlur={(e) =>
                            handleUpdate(stage.id, e.target.value)
                        }
                    />
